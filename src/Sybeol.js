...
var infoIp = function(msg, req) {
    logger.info("["+req.connection.remoteAddress+"] " + "["+req.session.id+"] " + msg);
}
var warnIp = function(msg, req) {
    logger.warn("["+req.connection.remoteAddress+"] " + "["+req.session.id+"] " + msg);
}
...
app.get('/measure', function(req, res) {
    ...
    authSvc.getCurrentUser(req, function(err, user) {
        if(err) {
            ...
            measureManager.list(user, "foo", function(err, measures) {
                if(err) {
                    throw new Error(err);
                } else {
                    res.writeHead(200, {'Content-Type': 'application/json'});
                    res.send(JSON.stringify(measures));
                }
            });
        }
    });
...
app.get('/measure/:device', function(req, res) {
    ...
    authSvc.getCurrentUser(req, function(err, user) {
        if(err) {
            ...
            measureManager.list(user, device, function(err, measures) {
                if(err) {
                    throw new Error(err);
                } else {
                    res.writeHead(200, {'Content-Type': 'application/json'});
